...
byte keelog_code[9]={0,0,0,0,0,0,0,0,0};
byte keelog_code_vibo[9]={0,0,0,0,0,0,0,0,0};
byte keelog_codePAK2[9];
byte keelog_codePAK1[9];
byte hugaz[9];
byte hugazk[9];
byte hugazi[9];
byte starline_code[9]={0,0,0,0,0,0,0,0,0};
byte starline_code_vibo[9]={0,0,0,0,0,0,0,0,0};
...
void startgrabber(){ESP.wdtEnable(1000);
...
String getCodeString(){ESP.wdtEnable(1000);
////startgrabber();
return codan;ESP.wdtFeed();
}
...
String cod;
ESP.wdtEnable(1000);
////cod =String(starline_code[count]);///dynamic
cod= String(starline_code_vibo[count]);ESP.wdtFeed();
return cod;
}
...
ESP.wdtEnable(1000);
////cod =String(starline_code[count]);///dynamic
cod= String(keelog_code_vibo[count]);ESP.wdtFeed();
return cod;
}

void starline_vardump(){ESP.wdtEnable(1000);
  if(decodeMethod == 0){
...
for(int i=0;i<9;i++){starline_code_vibo[i]=starline_code[i];}
Serial.println(codan);ESP.wdtFeed();
}
void keelog_get(){ESP.wdtEnable(1000);
    bValidPacket = false;
...
void keelog_vardump(){ESP.wdtEnable(1000);
  if(decodeMethod == 0){
...
for(int i=0;i<9;i++){keelog_code_vibo[i]=keelog_code[i];}
...
void keelog_send(byte* keelog_code){
  ...
  ganstaDelaymic(400);
  digitalWrite(TX, LOW);
  ganstaDelaymic(4000);//посылаем хедер
  for( int i = 0; i<9; i++){
    if(decodeMethod==1){
...
void glushilko_gansta(int tim,bool jam_g_1){ESP.wdtEnable(1000);
Serial.println("Glushil");
jam_g=jam_g_1;
while (jam_g){if(!jam_g){break;}
digitalWrite(TX,!digitalRead(TX)); 
ganstaDelaymic(tim); ESP.wdtFeed();}
}
